...
- [ ] 8. ジェットパックとジャンプ機能
  ...
  - [ ] 8.1 ジェットパック機能を実装
    ...
    - **手動確認**:
      - [ ] `npm run dev` でゲームを起動
      - [ ] 鬼モードでSPACEキーを長押しして飛行することを確認
      - [ ] 飛行中に燃料が減少することを確認
      - [ ] 燃料が0になると飛行できなくなることを確認
      - [ ] 地面に着地すると燃料が回復することを確認
  - [ ] 8.2 ジャンプ機能を実装
    - 逃げる側用のジャンプ（SPACE、表面上のみ）
    - 燃料消費なし
    - _要件: 4.4_
    - **手動確認**:
      - [ ] 逃げる側モードでSPACEキーを押してジャンプすることを確認
      - [ ] ジャンプ中に燃料が減少しないことを確認
      - [ ] 空中ではジャンプできないことを確認
  - [ ] 8.3 燃料システムを実装
    - 燃料ゲージ（0-100）
...
- [ ] 9. ダッシュ能力
  ...
  - [ ] 9.1 ダッシュ機能を実装
    ...
    - **手動確認**:
      - [ ] `npm run dev` でゲームを起動
      - [ ] 逃げる側モードでSHIFTキーを長押ししてダッシュすることを確認
      - [ ] ダッシュ中に移動速度が速くなることを確認
      - [ ] ダッシュ中に燃料が減少することを確認
      - [ ] 燃料が0になるとダッシュできなくなることを確認
  - [ ] 9.2 鬼のダッシュ制限を実装
    - 鬼はダッシュ不可
    - _要件: 5.5_
    - **手動確認**:
      - [ ] 鬼モードでSHIFTキーを押してもダッシュしないことを確認
  - [ ] 9.3 ダッシュ機能のテストを作成
    - ダッシュ速度のテスト
    - 燃料消費のテスト
    - 鬼の制限のテスト
...
- [ ] 10. ビーコン能力
  ...
  - [ ] 10.1 ビーコンシステムを実装
    ...
    - **手動確認**:
      - [ ] `npm run dev` でゲームを起動
      - [ ] 鬼モードでBキーを押してビーコンを起動することを確認
      - [ ] ビーコン起動時に全逃げる側の位置が表示されることを確認
      - [ ] 10秒後にビーコンが終了することを確認
      - [ ] 30秒のクールダウン中はビーコンが使用できないことを確認
      - [ ] クールダウンの進行状況が表示されることを確認
  - [ ] 10.2 ビーコンビジュアルを実装
    - 逃げる側の上に黄色のビーム
    - グローエフェクト
    - パルスアニメーション
    - _要件: 6.2, 11.6_
    - **手動確認**:
      - [ ] ビーコン起動時に逃げる側の上に黄色のビームが表示されることを確認
      - [ ] ビームがパルスアニメーションすることを確認
      - [ ] ビーコン終了時にビームが消えることを確認
  - [ ] 10.3 ビーコン機能のテストを作成
    - リチャージタイマーのテスト
    - 表示時間のテスト
...
- [ ] 11. カメラシステム
  ...
  - [ ] 11.1 一人称カメラを実装
    ...
    - **手動確認**:
      - [ ] `npm run dev` でゲームを起動
      - [ ] 一人称視点でプレイできることを確認
      - [ ] マウスを上下に動かしてカメラが上下に回転することを確認
      - [ ] 真上・真下を向けないことを確認（制限あり）
      - [ ] マウスを左右に動かしてカメラが左右に回転することを確認
  - [ ] 11.2 三人称カメラを実装
    - はしご登り時のカメラ
    - プレイヤーの後ろに配置
    - プレイヤーを見る
    - _要件: 15.2_
    - **手動確認**:
      - [ ] Cキーで三人称視点に切り替わることを確認
      - [ ] プレイヤーの後方からの視点であることを確認
      - [ ] プレイヤーが画面に表示されることを確認
      - [ ] Cキーで一人称視点に戻ることを確認
  - [ ] 11.3 カメラスムージングを実装
    - 地上移動時の水平レベリング
...
- [ ] 12. 視覚的インジケーターとエフェクト
  ...
  - [ ] 12.1 プレイヤーマーカーを実装
    ...
    - **手動確認**:
      - [ ] `npm run dev` でゲームを起動
      - [ ] 各プレイヤーの上にマーカーが表示されることを確認
      - [ ] 鬼のマーカーが赤色であることを確認
      - [ ] 逃げる側のマーカーが緑色であることを確認
      - [ ] マーカーが回転アニメーションすることを確認
      - [ ] 移動中のマーカーが明るいことを確認
      - [ ] 静止中のマーカーが薄いことを確認
  - [ ] 12.2 パーティクルエフェクトを実装
    ...
    - _要件: 4.1, 5.1_
    - **手動確認**:
      - [ ] ジェットパック使用時に炎のパーティクルが表示されることを確認
      - [ ] ダッシュ使用時にトレイルパーティクルが表示されることを確認
      - [ ] パーティクルが自然に消えることを確認
  - [ ] 12.3 スポットインジケーターを実装
    - 逃げる側が鬼に見つかった時の表示
    - 「見つかった！」メッセージ
    ...
    - _要件: 11.5_
    - **手動確認**:
      - [ ] 逃げる側として鬼に近づくと「SPOTTED」が表示されることを確認
      - [ ] 鬼から離れると「SPOTTED」が消えることを確認
      - [ ] インジケーターがパルスアニメーションすることを確認
...
- [ ] 14. タグ付けメカニクス
  ...
  - [ ] 14.1 タグ付け判定を実装
    ...
    - **手動確認**:
      - [ ] `npm run dev` でゲームを起動
      - [ ] 鬼として逃げる側に近づくとタグ付けできることを確認
      - [ ] タグ付けされた逃げる側が鬼になることを確認
      - [ ] タグ付けした元の鬼が逃げる側になることを確認
  - [ ] 14.2 タグ付け通知を実装
    - トーストメッセージの表示
    - 「鬼になった！」「タグ付けした！」メッセージ
    - アニメーション
    - _要件: 7.4_
    - **手動確認**:
      - [ ] タグ付けされた時に「鬼になった！」メッセージが表示されることを確認
      - [ ] タグ付けした時に「タグ付けした！」メッセージが表示されることを確認
      - [ ] メッセージがアニメーションして消えることを確認
  - [ ] 14.3 鬼変換処理を実装
    - プレイヤーの色変更（緑→赤）
    - ビーコンリチャージの開始
    ...
    - _要件: 7.2, 7.5_
    - **手動確認**:
      - [ ] 鬼になった時にマーカーが赤色に変わることを確認
      - [ ] 鬼になった時にビーコンのクールダウンが開始することを確認
      - [ ] 鬼になった時にダッシュが使えなくなることを確認
      - [ ] 鬼になった時にジェットパックが使えるようになることを確認
...
- [ ] 15. ゲームタイマーと終了条件
  ...
  - [ ] 15.1 タイマーシステムを実装
    ...
    - **手動確認**:
      ...
      - [ ] HUDにタイマーが表示されることを確認
      - [ ] タイマーがカウントダウンすることを確認
      - [ ] 残り60秒未満でタイマーが黄色になることを確認
      - [ ] 残り30秒未満でタイマーが赤色になることを確認
  - [ ] 15.2 ゲーム終了判定を実装
    - タイマーゼロでゲーム終了
    - 全プレイヤーが鬼でゲーム終了
    - ゲーム終了フラグの設定
    - _要件: 9.5, 9.6_
    - **手動確認**:
      - [ ] タイマーが0になるとゲームが終了することを確認
      - [ ] 全プレイヤーが鬼になるとゲームが終了することを確認
  - [ ] 15.3 ゲーム終了処理を実装
    - 結果画面への遷移
    - 統計の更新
    - _要件: 9.7_
    - **手動確認**:
      - [ ] ゲーム終了時に結果画面が表示されることを確認
      - [ ] 結果画面に各プレイヤーの生存時間が表示されることを確認
      - [ ] 最長生存者がハイライトされることを確認
...
- [ ] 17. UIシステム - HUDとゲーム内表示
  ...
  - [ ] 17.1 HUDを実装
    ...
    - **手動確認**:
      ...
      - [ ] HUDが画面に表示されることを確認
      - [ ] タイマーが表示されることを確認
      - [ ] 鬼と逃げる側の人数が表示されることを確認
      - [ ] 燃料ゲージが表示されることを確認
      - [ ] 自分のステータス（鬼/逃げる側）が表示されることを確認
      - [ ] 燃料を使用するとゲージが減ることを確認
      - [ ] 燃料が回復するとゲージが増えることを確認
  - [ ] 17.2 デバッグ情報を実装
    - F3キーでデバッグモード切り替え
    - 位置、速度の表示
    - 鬼情報、AIプレイヤー情報
    - _要件: なし（開発用）_
    - **手動確認**:
      - [ ] F3キーでデバッグ情報が表示されることを確認
      - [ ] プレイヤーの位置が表示されることを確認
      - [ ] プレイヤーの速度が表示されることを確認
      - [ ] F3キーでデバッグ情報が非表示になることを確認
  - [ ] 17.3 HUDのテストを作成
    - タイマー表示のテスト
    - プレイヤーカウントのテスト
    - _要件: 9.2_
...
- [ ] 18. UIシステム - 結果と統計
  ...
  - [ ] 18.1 結果画面を実装
    ...
    - _要件: 10.1, 10.2, 10.3_
    - **手動確認**:
      - [ ] `npm run dev` でゲームを起動してゲームを終了させる
      - [ ] 結果画面が表示されることを確認
      - [ ] プレイヤーリストが生存時間順に表示されることを確認
      - [ ] 最長生存者がハイライトされることを確認
      - [ ] 各プレイヤーの生存時間が表示されることを確認
      - [ ] 「メニューに戻る」ボタンでタイトル画面に戻ることを確認
  - [ ] 18.2 統計システムを実装
    - localStorageへの統計保存
    - 総ゲーム数、勝利数、敗北数
    - 勝率計算
...
- [ ] 19. モバイルコントロール
  ...
  - [ ] 19.1 タッチボタンを実装
    ...
    - **手動確認（モバイルデバイスまたはChrome DevToolsのモバイルモード）**:
      - [ ] モバイルデバイスまたはブラウザのモバイルモードで `npm run dev` を開く
      - [ ] 画面にタッチボタンが表示されることを確認
      - [ ] ジャンプ/ジェットパックボタンが表示されることを確認
      - [ ] ダッシュボタンが表示されることを確認（逃げる側のみ）
      - [ ] ビーコンボタンが表示されることを確認（鬼のみ）
      - [ ] 各ボタンをタップして機能することを確認
  - [ ] 19.2 ボタンの視覚的フィードバックを実装
    ...
    - _要件: 14.5_
    - **手動確認（モバイル）**:
      - [ ] ボタンをタップした時にスケールアニメーションすることを確認
      - [ ] 燃料が0の時にボタンがグレーアウトすることを確認
      - [ ] 鬼モード時にボタンの色が変わることを確認
